...
class EnvironmentAnalyzer:
    ...
    def _generate_dockerfile(
        ...
        """Generate a Dockerfile — runtime environment only.

        The image provides runtimes, global tools, and Claude Code CLI.
        Project code and dependencies are handled at runtime when the
        compute clones the repo from serving's Git infrastructure.
        """
        lines = [
            f"FROM {base_image}",
            "",
            "# Generated by ClaudeVN Layer 1 planning",
            "# Runtime environment only — project code cloned at runtime",
            "",
            "WORKDIR /workspace",
            "",
        ]
        ...
        if base_image.startswith("ubuntu"):
            ...
            # Node.js 22 LTS via nodesource (ubuntu's nodejs package is too old for Claude CLI)
            if "node" in runtimes:
                lines.extend([
                    "# Node.js 22 LTS",
        ...
        lines.extend([
            ...
            "",
            '# Project code is cloned at runtime via serving Git infrastructure',
            '# Project dependencies installed after clone (pip install, npm install, etc.)',
            "",
            'CMD ["claude"]',
        ])
