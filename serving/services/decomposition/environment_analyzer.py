...
class EnvironmentAnalyzer:
    ...
    def _detect_package_installs(self, runtimes: Set[str]) -> List[RuntimeRequirement]:
        """Detect package install commands needed."""
        requirements = []
        ...
        if "python" in runtimes:
            ...
            if req_path:
                ...
                requirements.append(RuntimeRequirement(
                    ...
                    reason=f"{rel} exists",
                    install_cmd=f"pip install -r {rel}",
                ))
        ...
        if "node" in runtimes:
            ...
            if pkg_path:
                ...
                requirements.append(RuntimeRequirement(
                    name="node-packages",
                    reason=f"{pkg_dir}/package.json exists",
                    install_cmd=f"cd {pkg_dir} && npm install" if pkg_dir != "." else "npm install",
                ))
    ...
    def _generate_dockerfile(
        ...
        """Generate a Dockerfile from detected requirements."""
        lines = [
            f"FROM {base_image}",
            "",
        ...
        # Multi-runtime: install each runtime
        ...
        install_cmds = [r.install_cmd for r in requirements if r.install_cmd]
        if install_cmds:
            lines.append("# Project dependencies and tools")
