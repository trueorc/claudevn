...
# =============================================================================
# Test: PRService safe.directory bypass (Issue #848)
# =============================================================================

class TestPRServiceSafeDirectory:
    """Test that PRService git operations bypass safe.directory checks.

    Serving runs as root but repos are owned by git user (CVE-2022-24765).
    All raw git subprocess calls must include the safe.directory env bypass.
    """

    @pytest.fixture
    def pr_service(self):
        """Create PRService for safe.directory tests."""
        with patch("git.pr_service.get_config") as mock_config:
            mock_config.return_value.git.repos_path = "/home/git/repos"
            service = PRService(
                redis_client=None,
    ...
    def test_safe_git_env_sets_safe_directory(self, pr_service):
        """Test _safe_git_env returns env with safe.directory config."""
        env = pr_service._safe_git_env(Path("/home/git/repos/project.git"))

        assert env["GIT_CONFIG_COUNT"] == "1"
        assert env["GIT_CONFIG_KEY_0"] == "safe.directory"
        assert env["GIT_CONFIG_VALUE_0"] == "/home/git/repos/project.git"

    def test_git_cmd_passes_safe_env(self, pr_service):
        """Test _git_cmd passes safe.directory env to subprocess."""
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")

            pr_service._git_cmd(
                Path("/home/git/repos/project.git"),
                "merge-base", "main", "feature/test"
            )
        ...
        assert call_kwargs[0][0] == [
            "git", "-C", "/home/git/repos/project.git",
            "merge-base", "main", "feature/test"
        ]

    @pytest.mark.asyncio
    async def test_check_mergeable_uses_safe_env(self, pr_service):
        """Test check_mergeable passes safe.directory env to git merge-base."""
        pr_service._repo_manager.get_branch_head = MagicMock(
            side_effect=lambda p, b: {"main": "abc", "feature/test": "def"}.get(b)
        )
        ...
        with patch("pathlib.Path.exists", return_value=True):
            with patch("subprocess.run") as mock_run:
                ...
                await pr_service.check_mergeable("project", "feature/test")

        # Verify env was passed with safe.directory
        call_kwargs = mock_run.call_args
        assert "env" in call_kwargs[1]
        assert call_kwargs[1]["env"]["GIT_CONFIG_KEY_0"] == "safe.directory"

    @pytest.mark.asyncio
    async def test_dry_run_merge_clone_uses_safe_env(self, pr_service):
        """Test dry_run_merge passes safe.directory env to git clone."""
        pr_service._repo_manager.get_branch_head = MagicMock(return_value="abc123")
        ...
        with patch("subprocess.run", side_effect=capture_subprocess):
            with patch("shutil.rmtree"):
                with patch("pathlib.Path.exists", return_value=True):
                    with patch("pathlib.Path.mkdir"):
                        await pr_service.dry_run_merge("project", "feature/test")

        # All subprocess calls should have env with safe.directory
        assert len(captured_envs) > 0
        for env in captured_envs:
            assert env is not None, "subprocess.run called without env"
            assert env.get("GIT_CONFIG_KEY_0") == "safe.directory"

    @pytest.mark.asyncio
    async def test_merge_clone_uses_safe_env(self, pr_service):
        """Test merge passes safe.directory env to all git subprocess calls."""
        mock_redis = AsyncMock()
        mock_redis.get_branch_status = AsyncMock(return_value={
            "status": "approved",
            "compute_id": "compute-1",
            "task_id": "task-1",
            "head_commit": "abc123"
        })
        mock_redis.set_branch_status = AsyncMock()
        mock_redis.remove_from_pr_queue = AsyncMock()
        mock_redis.publish_git_event = AsyncMock()
        mock_redis.untrack_compute_branch = AsyncMock()
        mock_redis.get_pr_queue_position = AsyncMock(return_value=1)
        pr_service._redis = mock_redis
        pr_service._repo_manager.get_branch_head = MagicMock(return_value="abc123")
        ...
        with patch("subprocess.run", side_effect=capture_subprocess):
            with patch("shutil.rmtree"):
                with patch("pathlib.Path.exists", return_value=True):
                    with patch("pathlib.Path.mkdir"):
                        await pr_service.merge("project", "feature/test")
